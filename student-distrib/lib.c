...
*   Inputs: void
*   Return Value: none
*	Function: clears screen, makes screen_x and screen_y
*/
void
reset_screen() {
    clear();
...
putc(uint8_t c)
{
  ...
  // sti();
    ...
    if(c == '\n' || c == '\r') {
      ...
      if (screen_y > NUM_ROWS-1) {
          for(i = 0; i < NUM_ROWS-1; i++){
            memcpy(video_mem + NUM_COLS * i * 2, video_mem + NUM_COLS * (i+1) * 2, NUM_COLS*2);
          }
          screen_y--;
          for(i = 0; i < NUM_COLS; i++)
          *(uint8_t *)(video_mem + ((NUM_COLS*screen_y + i) << 1)) = ' ';
          *(uint8_t *)(video_mem + ((NUM_COLS*screen_y + i) << 1) + 1) = ATTRIB_YELLOW;
    ...
    else if(tbuf == 128) return;
    else {
        *(uint8_t *)(video_mem + ((NUM_COLS*screen_y + screen_x) << 1)) = c;
        *(uint8_t *)(video_mem + ((NUM_COLS*screen_y + screen_x) << 1) + 1) = ATTRIB_YELLOW;

        screen_x++;

        // go to next line if we run out horizontal space
        if (screen_x >= NUM_COLS) {
            screen_y++;
            screen_x=0;

            //  shift video memory if we run out of vertical space
            if (screen_y > NUM_ROWS-1) {
                for(i = 0; i < NUM_ROWS-1; i++){
                  memcpy(video_mem + NUM_COLS * i * 2, video_mem + NUM_COLS * (i+1) * 2, NUM_COLS*2);
                }
